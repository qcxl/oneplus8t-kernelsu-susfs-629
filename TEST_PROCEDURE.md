...
每次开始新项目（Magisk 模块支持、KPM 移植等），按以下步骤配置项目上下文：

### 步骤 1：评估第 I 部分的适用性
- **移植前检查清单（第1-5节）**：完全通用，无需修改 ✅
- **事后复盘流程**：完全通用，无需修改 ✅
- **刷机流程**：[FLASH_PROCEDURE.md](FLASH_PROCEDURE.md) 中的命令需要按设备调整（检查分区名、boot.img 大小等）
- **pre-flight-check.sh**：Kconfig 命名空间前缀可能需要修改（`CONFIG_KSU_SUSFS_` → 新项目前缀）

### 步骤 2：填写项目上下文（复制以下模板到此处并填写）
...
- **目标版本**：［版本号］（来源）
- **内核版本**：［版本号］（平台）
- **基础框架**：［例如 KSUN-legacy、tiann/KernelSU 等］
...
### Kconfig 命名空间
- 前缀：［例如 `CONFIG_KSU_SUSFS_`］
- 定义位置：［配置文件路径］ + ［CI workflow 注册位置］
...
### 步骤 3：交叉检查
- [ ] FLASH_PROCEDURE.md 中的设备分区名、工具命令与当前设备匹配
- [ ] pre-flight-check.sh 的 Kconfig 命名空间前缀已更新
- [ ] 第 I 部分方法论中的示例代码（如 `susfs_add_*`）替换为新项目的实际函数名
- [ ] ERRORS.md 清空或保留跨项目通用的经验

---

> **以下为当前项目的上下文。新项目时此段按以上指南替换。**

...
'''
            ...
            tw_new = (
                ...
                '\t\t * On LineageOS 20, Bionic doesn\\x27t set PR_SET_SECCOMP\n'
                '\t\t * for regular app UIDs, so seccomp.mode stays 0.\n'
                '\t\t * We\\x27re in task_work context (process context, safe for\n'
                '\t\t * GFP_KERNEL in prctl_set_seccomp).\n'
                '\t\t * This fixes \"Seccomp: 已禁用\" on KSU-Next home page. */\n'
                '\t\tif (current->seccomp.mode == 0) {\n'

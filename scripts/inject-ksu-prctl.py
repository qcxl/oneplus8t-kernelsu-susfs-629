...
'''
            ...
            tw_new = (
                ...
                '\t\tif (bmp_uid < KSU_BMP_MAX_UID)\n'
                '\t\t\tset_bit((unsigned int)bmp_uid, ksu_seccomp_bmp);\n'
                '\t\t/* Daemon dies when parent App is killed.\n'
                '\t\t * Avoids orphan libksud processes spinning at 100%% CPU.\n'
                '\t\t * NOTE: Seccomp is NOT installed here (daemon doesn\\x27t need it).\n'
                '\t\t * App process gets seccomp via do_get_info IOCTL path. */\n'
                '\t\tcurrent->pdeath_signal = SIGKILL;\n'
                '\t}\n'
                '\tif (copy_to_user(tw->outp, &fd, sizeof(fd)))')

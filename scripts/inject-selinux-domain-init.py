...
def fix_rules(kernel_root):
    ...
    new = (
        ...
        '    /* Core diag: is KSU policy wrapping kernel policydb? */\n'
        '    printk(KERN_ERR "KSU_DIAG: old_pol_pdb=%px ss_pdb=%px same=%d orig_len=%zu jiffies=%lu\\n",\n'
        '        &old_pol->policydb, &selinux_state.ss->policydb,\n'
        '        &old_pol->policydb == (void*)&selinux_state.ss->policydb,\n'
        '        old_pol->policydb.len, jiffies);\n'
...
def fix_selinux_load_policy_kprobe(kernel_root):
    ...
    kprobe_code = (
        ...
        '{\n'
        '\tprintk(KERN_ERR "KSU_DIAG: workfn start jiffies=%lu\\n", jiffies);\n'

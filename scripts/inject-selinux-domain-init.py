...
def fix_selinux_load_policy_kprobe(kernel_root):
    ...
    kprobe_code = (
        '\n'
        '/* KSU_SECPOLICY_KPROBE: kprobe on sel_write_load.\n'
        ' * Pre_handler reads count (3rd param, x2 on ARM64) to log\n'
        ' * the actual policy write size. Post_handler checks if\n'
        ' * real policy was loaded and schedules work to create ksu domain. */\n'
        '\n'
        ...
        '\n'
        'static int ksu_sel_write_load_pre(struct kprobe *p,\n'
        '\t\t\t\t    struct pt_regs *regs)\n'
        '{\n'
        ...
        '\t\t p1, p2, p3, p4);\n'
        '\tksu_diag_log(buf);\n'
        '\t/* Also try reading count directly from regs[2] */\n'
        '\tsnprintf(buf, sizeof(buf),\n'
        '\t\t "KSU_DIAG: sel_load regs[0-3]=%lx,%lx,%lx,%lx\\n",\n'
        '\t\t real_regs->regs[0], real_regs->regs[1],\n'
        '\t\t real_regs->regs[2], real_regs->regs[3]);\n'
        '\tksu_diag_log(buf);\n'
        '\treturn 0;\n'
        '}\n'
        '\n'
        'static void ksu_sel_write_load_post(struct kprobe *p,\n'
        '\t\t\t\t     struct pt_regs *regs,\n'
        '\t\t\t\t     unsigned long flags)\n'
        '{\n'
        '\tchar buf[64];\n'
        '\tsnprintf(buf, sizeof(buf),\n'
        '\t\t "KSU_DIAG: sel_load post len=%zu\\n",\n'
        '\t\t selinux_state.ss->policydb.len);\n'
        '\tksu_diag_log(buf);\n'
        '\tif (selinux_state.ss->policydb.len > 0) {\n'
        '\t\tksu_diag_log("KSU_DIAG: real policy, schedule work\\n");\n'
        '\t\tschedule_work(&ksu_selinux_policy_work);\n'
        '\t} else {\n'
        '\t\tksu_diag_log("KSU_DIAG: dummy/empty, skip\\n");\n'
        '\t}\n'
        '}\n'
        '\n'
        ...
        '\t.post_handler = ksu_sel_write_load_post,\n'
        '};\n'
        '\n'
        'static int __init ksu_sel_write_load_kprobe_init(void)\n'
        '{\n'
        '\tint rc = register_kprobe(&ksu_sel_write_load_kp);\n'
        '\tif (rc) {\n'
        '\t\tpr_err("KSU: sel_write_load kprobe failed: %d\\n", rc);\n'
        '\t} else {\n'
        '\t\tpr_info("KSU: sel_write_load kprobe registered\\n");\n'
        '\t\tksu_diag_log("KSU_DIAG: sel_write_load kprobe registered\\n");\n'
        '\t}\n'
        '\t/* If real policy already loaded at initcall time,\n'
        ...
        '\treturn 0;\n'
        '}\n'
        'late_initcall(ksu_sel_write_load_kprobe_init);\n'
    )

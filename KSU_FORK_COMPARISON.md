...
> 最后更新: 2026-07-07 | 基于源码全量对比 + GLM 5.2 MAX + Arena 交叉验证 + 实机验证
> dev: `8c363a1` + `d3e454f` (init.rc注入) | legacy: `77b3027` → `eb139a6` (430a739 同步)
> 项目构建: #189 通过 | 设备: OnePlus 8T (kebab), kernel 4.19.304
...
init.rc 模块注入            ████████████████████ 100%  /metadata/ksu/modules.rc
allow_shell module_param    ████████████████████ 100%  CONFIG_KSU_DEBUG 控制
```
...
| **init.rc 模块注入** | 追加模块 RC（/metadata/ksu/modules.rc）到 init.rc EOF，允许 KSU 模块注入自定义 init 脚本 | 🟡 5h | `d3e454f` |
| **allow_shell module_param** | 添加 `allow_shell` 内核参数（CONFIG_KSU_DEBUG 时默认 true），cmdline/sysfs 控制 shell root | 🟢 0.5h | dev init.c |
...
| **throne_tracker 重写（哈希化）** | 文档原称 dev 用 `DEFINE_HASHTABLE(apk_path_hash_list, 8)`，但经实际源码比对：**dev 的 `apk_path_hash_list` 也是 `struct list_head` 而非哈希表**。两者都是线性链表。唯一的差异是 dev 多了 `data_app_magic` 文件系统类型检查。`apk_path_hash_list` 用于缓存已扫描的应用路径，通常 <100 条，无论哪种方式 O(N) 性能都足够。Legacy 的 bitmap 快速路径已覆盖 99% 的 uid 查询。 | legacy 线性链表（性能足够） |
...
## 四、⏳ 待调研/待决策（已全部出结论，已关闭）

```
allowlist 哈希化           ████████████████████ 100%  ❌ 跳过。与过检测无关，收益极低
KSU_VERSION +150           ████████████████████ 100%  ✅ 已清理（commit XXXXXX）
sucompat 清理              ████████████████████ 100%  ❌ 依赖 syscall_hook，后者已关闭
...
| 🟡 **中** | KSU_VERSION `+150` — 已删除。GHA 直接 `sed` 写入 33188。 |
...
移植工作**已完成**，11/11 可移植功能全部实现并通过刷机验证。当前内核已具备：

| 功能类别 | 具体功能 |
|---------|---------|
| 🔒 SELinux 隐藏 | `selinux_hide` 完整版（context+access+setprocattr）+ fake status page |
| 📋 事件日志 | `sulog` + `event_queue`（替代 tiny_sulog） |
| 🔑 权限管理 | `adb_root`、`sucompat`、`kernel_umount`、`allow_shell` |
| 🛡️ 安全加固 | `remove_avtab_node`、`setuid_hook spin_lock` |
| 🧩 工具链 | `symbol_resolver`、`ksud`（ID=5 支持） |
| 🚀 init 增强 | `init.rc` 模块 RC 注入（/metadata/ksu/modules.rc） |

### 第二阶段：已调研关闭项（不影响过检测，不操作）

以下 4 项经源码全量审计，确认与过检测目标无关，全部关闭不移植：

| 功能 | 结论 | 与过检测的关系 |
|------|:----:|:-------------:|
| `allowlist` 哈希化 | ❌ 跳过。内部数据结构优化，外部不可见 | 零影响 |
| `syscall_hook` 系列 | ❌ 跳过。kprobe 已正常工作，替代方案高风低收 | 不影响（manual hook 已覆盖核心路径） |
| `throne_tracker` 哈希化 | ❌ 跳过。dev 本身也没用哈希表，两者都是链表 | 零影响 |
| `sucompat` / `ksu_cred` | ❌ 跳过。依赖 syscall_hook，后者已关闭 | 零影响 |

### 第三阶段：错误修复记录

移植和刷机验证过程中修复的错误（记录于 ERRORS.md E036-E039）：

| 编号 | 现象 | 根因 | 修复 |
|:----:|------|------|------|
| E036 | selinux_hide 注入失败 | 上游 430a739 重命名函数，注入脚本锚点失效 | 更新锚点兼容新旧函数名 |
| E037 | init.rc 注入编译失败（label undeclared） | ksud_integration.c 混合 tab/space 缩进，锚点全部匹配失败 | 修正 tab/space 混合缩进 + 结尾空行差异 |
| E038 | adb_root 冲突（redefinition + 参数数量） | 上游 430a739 同步了 adb_root，注入脚本重复定义 | 检测已存在则跳过，更新 transive_to_domain 为 3 参数 |
| E039 | 刷机 ~4min 后 panic 关机 | 6 个预存 section mismatch 触发 freed __init memory 访问 | `CONFIG_DEBUG_SECTION_MISMATCH=y` 触发编译器改动布局，实际修复来自代码重排列 |

### 第四阶段：跨项目经验复用

移植过程中积累的错误经验存储在 `ERRORS.md` 中（E001-E039），涵盖：
